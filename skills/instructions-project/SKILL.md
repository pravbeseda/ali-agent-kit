...
state (`absent`, `in-sync`, `drift`, `stale-import`, `not-a-shim`, `orphan`), the Copilot copy
...
detected" only for `agents-moved` / `hand-edited` / `both` / `stale-import`
(a shim from before the import-path fix: `@AGENTS.md` resolves to
`.claude/AGENTS.md`, so Claude Code never loaded the canon — re-render).
