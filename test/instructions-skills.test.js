...
test('render: global targets carry marker (+ frontmatter for Copilot), shim carries the AGENTS.md hash', async () => {
  const { renderGlobal, renderShim, renderCopilotCopy, stripRenderedHead, shimTail } = await lib('render.js');
  ...
  // the Claude-only tail survives a re-render, also from a stale-import shim
...
test('project flow: solo repo with root CLAUDE.md → AGENTS.md + shim, archive, drift, sync', () => {
  ...
  assert.equal(runScript(P, 'inventory.js', ['--new-run'], { home, cwd: repo }).json.shim.state, 'stale-import');
  writeFileSync(join(repo, '.claude', 'CLAUDE.md'), shim);
  ...
  // ...and keeps the Claude-only tail of the existing shim (#30)
  assert.match(readFileSync(join(ren2.json.proposalDir, 'claude-shim.md'), 'utf8'), /^<!-- instructions-project: shim; canonical AGENTS\.md sha256 [0-9a-f]{64} -->\n@\.\.\/AGENTS\.md\n\nUse the Claude `\/review` command before opening a PR\.\n$/);
  assert.doesNotMatch(drift.json.files[0].diff, /^-Use the Claude/m, 'drift --diff does not show the kept tail as removed');
