...
| `ali-review-pr-duo`              | Claude Code only. Reviews a pull request twice at once — `ali-review-pr` on Claude and on Codex, each in its own clean context — waits for both, then hands everything they posted to `ali-process-pr-comments`. Manual only: it publishes to the PR. |
