...
| Skill                            | What it does                                                                                                                                                                                      |
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ali-autopilot`                  | Claude Code only, manual-only. Implements a feature on its own — plans it, then runs a step → independent review → fix loop with subagents that never saw the reasoning behind the code, answers every finding with a written ruling, and stops only for a choice that changes how the application behaves or where it goes next. Ends with a branch, a pull request and the list of decisions it took for you. |
...
| `ali-review-pr`                  | Reviews a pull request against one bar — does the change leave the codebase healthier — posts what fails it as inline `blocking` / `suggestion` comments, and ends with a ready-to-merge verdict. On a follow-up round it also reopens its own threads that were resolved on a fix that does not cover what they raised. A round that posts no comment at all explains the PR in plain language instead. |
| `ali-review-pr-duo`              | Claude Code only. Reviews a pull request twice at once — `ali-review-pr` on Claude Opus and on Codex, each in its own clean context — waits for both, then hands everything they posted to `ali-process-pr-comments`. Manual only: it publishes to the PR. |
